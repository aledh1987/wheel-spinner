...
function defaultSpinner(name="My Spinner") { return { id:uid(), name, options:["Option 1","Option 2","Option 3"], history:[], removeWinner:false }; }
...
function drawWheel(canvas, options, rotation, winnerIdx=-1, glowPulse=0) {
  ...
  options.forEach((opt,i)=>{
    ...
    if(i===winnerIdx&&glowPulse>0){ ctx.shadowColor=COLORS[i%COLORS.length]; ctx.shadowBlur=14+28*glowPulse; }
    ...
    ctx.font=`bold ${fs}px sans-serif`; ctx.shadowColor="rgba(0,0,0,0.5)"; ctx.shadowBlur=4;
...
function SpinnerView({spinner,onUpdate,theme,soundRef}) {
  ...
  function onDrop(i,e){ e.preventDefault(); const src=dragSrcRef.current; if(src===null||src===i){setDragOver(null);return;} const next=[...spinner.options]; const[item]=next.splice(src,1); next.splice(i,0,item); onUpdate({options:next}); dragSrcRef.current=null; setDraggingIdx(null); setDragOver(null); }
  ...
  function importList(){
    ...
    if(incoming.length) onUpdate({options:[...spinner.options,...incoming]});
  ...
  function spin(){
    ...
    const opts=[...spinner.options], slice=getSliceAngle(opts.length);
    ...
    function frame(now){
      ...
      function gloop(ts){ const el=ts-gs; const pulse=el<GDUR?(Math.sin(el/290*Math.PI*2)+1)/2:0.65; if(canvasRef.current)drawWheel(canvasRef.current,opts,rotRef.current,wIdx,pulse); if(el<GDUR+60){glowRef.current=requestAnimationFrame(gloop);}else{winIdxRef.current=-1;if(canvasRef.current)drawWheel(canvasRef.current,spinnerRef.current.options,rotRef.current);} }
  ...
  const tabBtn=(id,label)=>({ onClick:()=>setTab(id), style:{ flex:1,padding:"7px",border:"none",borderRadius:8,fontFamily:"inherit",fontSize:12,fontWeight:600,cursor:"pointer",transition:"all 0.15s", background:tab===id?`${theme.accent}99`:"transparent", color:tab===id?"#fff":"rgba(255,255,255,0.4)" } });
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",padding:"20px 16px 40px",gap:16,overflowY:"auto"}}>
      ...
      <div style={{width:"100%",maxWidth:360}}>
        <div style={{display:"flex",gap:3,marginBottom:12,background:"rgba(255,255,255,0.05)",borderRadius:10,padding:3}}>
          <button {...tabBtn("options",`Options (${options.length})`)}>Options ({options.length})</button>
          <button {...tabBtn("history",`History (${history.length})`)}>History ({history.length})</button>
        </div>
        ...
        {tab==="options"?(
          <div style={{display:"flex",flexDirection:"column",gap:9}}>
            ...
            <button
              ...
              style={{display:"flex",alignItems:"center",gap:6,background:showImport?theme.soft:"rgba(255,255,255,0.06)",border:`1px solid ${showImport?theme.border:"rgba(255,255,255,0.1)"}`,color:theme.muted,borderRadius:8,padding:"7px 12px",cursor:"pointer",fontFamily:"inherit",fontSize:12,fontWeight:600,transition:"all 0.15s"}}
            >
              <span style={{fontSize:14}}>{showImport?"✕":"📋"}</span>
              {showImport?"Close import":"Import a list"}
            </button>
            ...
            {showImport&&(
              <div style={{display:"flex",flexDirection:"column",gap:8,padding:"12px",background:"rgba(255,255,255,0.04)",borderRadius:10,border:`1px solid ${theme.border}`}}>
                ...
                <textarea
                  ...
                  rows={4}
                  style={{background:"rgba(255,255,255,0.07)",border:"1.5px solid rgba(255,255,255,0.15)",borderRadius:8,padding:"9px 12px",color:"#fff",fontFamily:"inherit",fontSize:13,outline:"none",resize:"vertical",width:"100%"}}
                />
                <div style={{display:"flex",gap:7}}>
                  <button
                    onClick={importList}
                    disabled={!importText.trim()}
                    style={{flex:1,background:importText.trim()?theme.soft:"rgba(255,255,255,0.05)",border:`1px solid ${theme.border}`,color:importText.trim()?theme.muted:"rgba(255,255,255,0.3)",borderRadius:100,padding:"8px",cursor:importText.trim()?"pointer":"default",fontFamily:"inherit",fontSize:12,fontWeight:700,transition:"all 0.15s"}}
                  >
                    ✓ Add all items
                  </button>
            ...
            {/* ── Options list (draggable) ── */}
            {options.length>0&&<p style={{margin:"4px 0 0",fontSize:11,color:"rgba(255,255,255,0.3)"}}>⠿ Drag rows to reorder</p>}
            <div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:260,overflowY:"auto"}}>
              {options.map((opt,i)=>(
                <div
                  ...
                  onDragEnd={onDragEnd}
                  style={{
                    display:"flex",alignItems:"center",gap:7,padding:"7px 9px 7px 5px",
                    background:dragOver===i?theme.soft:"rgba(255,255,255,0.05)",
                    borderRadius:9,
                    border:`1px solid ${dragOver===i?theme.border:"rgba(255,255,255,0.07)"}`,
                    opacity:draggingIdx===i?0.3:1,
                    cursor:"grab",
                    transition:"background 0.1s,border-color 0.1s,opacity 0.1s",
                    boxShadow:dragOver===i?`0 0 0 2px ${theme.border}`:"none",
                  }}
                >
                  ...
                  {editIdx===i?(
                    <input autoFocus value={editVal} onChange={e=>setEditVal(e.target.value)}
                      ...
                      style={{flex:1,background:"rgba(255,255,255,0.1)",border:`1px solid ${theme.border}`,borderRadius:5,color:"#fff",fontFamily:"inherit",fontSize:12,padding:"2px 7px",outline:"none"}}/>
                  ):(
                    <span style={{flex:1,fontSize:12,color:"rgba(255,255,255,0.85)"}}>{opt}</span>
                  )}
                  <button onClick={()=>{setEditIdx(i);setEditVal(opt);}} title="Edit" style={{background:"none",border:"none",color:"rgba(255,255,255,0.3)",cursor:"pointer",fontSize:12,padding:"2px 3px"}}>✏️</button>
                  <button onClick={()=>removeOption(i)} title="Remove" style={{background:"none",border:"none",color:"rgba(255,107,107,0.5)",cursor:"pointer",fontSize:15,padding:"2px 3px",lineHeight:1}}>×</button>
                </div>
              ))}
            ...
            {options.length>0&&<button onClick={()=>onUpdate({options:[]})} style={{background:"rgba(255,107,107,0.1)",border:"1px solid rgba(255,107,107,0.2)",color:"#ff6b6b",borderRadius:8,padding:"7px",cursor:"pointer",fontSize:11,fontFamily:"inherit",fontWeight:600}}>Clear all options</button>}
          </div>
        ):(
...
export default function App() {
  const [spinners,setSpinners]=useState(()=>{
    try{ const r=localStorage.getItem("wheel-v3"); if(r){const d=JSON.parse(r);if(Array.isArray(d)&&d.length>0)return d.map(s=>({history:[],removeWinner:false,...s}));} }catch{}
    ...
    <div style={{display:"flex",minHeight:"100vh",background:theme.bg,fontFamily:"'Segoe UI',sans-serif",color:"#fff"}}>
      <style>{`
        *{box-sizing:border-box;}
        ...
        .spin-btn:hover:not(:disabled){transform:translateY(-2px) scale(1.04);box-shadow:0 12px 36px ${theme.spinShadow};}
